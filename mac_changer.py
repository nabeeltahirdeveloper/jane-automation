...
import string
import random
# from mac_vendor import mac_addres
...
def get_user_adapter_choice(connected_adapters_mac):
    ...
    try:
        choice = int(
            input("Please choose the interface you want to change the MAC address:"))
...
def change_mac_address(adapter_transport_name, new_mac_address):
    ...
    output = subprocess.check_output(
        f"reg QUERY " + network_interface_reg_path.replace("\\\\", "\\")).decode()
...
if __name__ == "__main__":
    ...
    old_mac_address, target_transport_name = get_user_adapter_choice(
        connected_adapters_mac)
